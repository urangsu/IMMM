...
확인:
- [ ] `stroke.widthNorm`이 저장되고 export에서 `widthNorm * w` 로 사용되는지
- [ ] Sparkle brush spacing이 그리는 속도와 무관하게 일정한지
- [ ] Sparkle이 export에서 round pen으로 fallback되지 않는지
- [ ] `sticker.payload.sizeNorm`이 저장되고 export에서 `sizeNorm * baseW`로 사용되는지
- [ ] Text sticker size가 preview/export에서 동일하게 보이는지
- [ ] `document.fonts.ready` 이후 export하는지
- [ ] Pointer move 중 canvas가 즉시 갱신되는지 (`drawVersion`)
- [ ] Desktop Chrome / MacBook M1 Chrome / iPhone Safari에서 동일 확인
