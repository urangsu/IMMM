...
이번 PR에서는 대규모 리팩터링하지 말고 원인 조사 + 최소 수정만 한다.

문제:
- sparkle brush가 preview와 저장본에서 다르게 보임
- pen thickness가 preview와 저장본에서 다름
- text가 저장본에서 작아지거나 위치가 달라짐

확인:
- [ ] drawStrokes points가 0~100 normalized 좌표인지
- [ ] preview와 export가 같은 drawStrokes 데이터를 쓰는지
- [ ] stroke.width가 preview/export에서 같은 scale 공식을 쓰는지
- [ ] sparkle brush가 export에서 round brush fallback 되지 않는지
- [ ] text sticker size가 preview/export에서 같은 scale 공식인지
- [ ] document.fonts.ready 이후 export하는지
