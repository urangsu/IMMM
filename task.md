...
- [x] PR 3 — Mask debug overlay (Mode 1/2/3) 및 다중 인원(faces[]) 대응 완료
- [x] PR 4 — Cream Skin 품질 튜닝 완료 (Strength 0.32 / YCbCr 임계값 최적화)
- [ ] PR 5 — Mobile opt-in (Downsample FBO 구현 및 성능 검증 전까지 금지)

### 🧪 Smooth Filter (Cream Skin) Strength Comparison
실험 단계에서 최적의 화질을 위해 비교 가능하도록 함:
- **0.25 (Subtle)**: 아주 약한 보정, 눈에 띄지 않으나 피부톤이 미세하게 정리됨.
- **0.32 (Balanced)**: **현재 기본값**. 대부분의 조명에서 자연스러운 피부 질감을 유지하며 잡티 제거.
- **0.40 (Strong)**: 조명이 강한 환경에서 매끈한 피부 표현. 어두운 곳에서는 과할 수 있음.
