...
### 2026-05-03 / MacBook / Chrome / commit 99c50f0
- **App load**: Pass (no errors)
- **Step 1 preset sticker**: Pass (Stickers appear visually appropriately sized in 160~360px CSS containers via `sizeNorm` resolution).
- **Deco Minimal heart visual size**: Pass (Visually aligns perfectly across Step 1 and Deco Studio via `normScale`, matching final export).
- **Deco Minimal heart hit target**: Pass (minimum 24px applied, drag/select works smoothly)
- **Deco IMMM logo visual size**: Pass (displays correctly without overflow, matching visual ratios in both layout and result exports)
- **Deco Handwritten sticker**: Pass (text `sizeNorm` correctly integrates with the unified `normScale` rendering pipeline)
- **Controls size**: Pass (handles remain fixed at ~28px screen size despite visual/base scale separation)
- **Move/scale/rotate**: Pass (transforms apply seamlessly using updated effectiveScale values)
- **Save result**: Pass (native sticker coordinates map precisely to the high-res 880px+ canvas using the resolved native `normScale`)
- **Console errors**: Pass
- **Pass/Fail**: Pass
- **Notes**: Abstracted visual footprint into `sizeNorm` (CSS visual ratio relative to container), completely solving the mismatched sticker magnitudes between Step 1 layout, Deco Studio overlay, and final export canvas.

### 2026-05-03 / MacBook / Chrome / commit 31c85be
- **App load**: Pass (no errors)
- **Step 1 preset sticker**: Pass (hitbox/outline/controls are normal)
- **Deco Minimal heart visual size**: Pass (appears visually small at ~11px on screen, matches scaled native coords)
- **Deco Minimal heart hit target**: Pass (minimum 24px applied, drag/select works smoothly)
- **Deco IMMM logo visual size**: Pass (displays correctly without overflow, ~18px width on screen)
- **Deco Handwritten sticker**: Pass (size matches visual layout)
- **Controls size**: Pass (handles remain fixed at ~28px screen size)
- **Move/scale/rotate**: Pass (transforms apply from visual center seamlessly)
- **Save result**: Pass (native sticker coordinates map accurately to high-res export)
- **Console errors**: Pass (none, except intended IMMM_DEBUG_STICKER logs if enabled)
- **Pass/Fail**: Pass
- **Notes**: Separating `visualW` outline from the `24px` minimum hit target resolved the interaction difficulty for mini stickers without destroying layout accuracy.
