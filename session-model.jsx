...
(function() {
  ...
  const validateFrameReadiness = ({ layout, shots, selected }) => {
    const errors = [];
    if (!layout) {
    ...
    if (typeof window !== 'undefined' && typeof window.getLayoutSlotCount === 'function') {
      ...
      if (layout && (layout.includes('1x4') || layout.includes('strip'))) slotCount = 4;
      else if (layout && layout.includes('2x2')) slotCount = 4;
      else if (layout && layout.includes('wide')) slotCount = 3;
      else if (layout && layout.includes('polaroid')) slotCount = 1;
      else if (layout && layout.includes('trip')) slotCount = 3;
      else if (layout && layout.includes('grid')) slotCount = 4;
    }
    ...
    selected.forEach(c => {
      const idxVal = c.targetSlotIndex !== undefined ? c.targetSlotIndex : c.slotIndex;
    ...
    selected.forEach((cut, i) => {
      const targetAssetId = cut.assetId;
      let asset = null;
      ...
      if (!asset && cut.sourceShotIndex !== undefined && cut.sourceShotIndex !== null) {
        const fallbackIdx = Number(cut.sourceShotIndex);
      ...
      if (!asset && targetAssetId && targetAssetId.startsWith('asset_')) {
        const parsed = Number(targetAssetId.replace('asset_', ''));
      ...
      if (!asset) {
        ...
        const hasImage = (typeof asset.dataUrl === 'string' && asset.dataUrl.trim().length > 0) ||
