...
const CREATOR_PROFILE_STORAGE_KEY = 'immm.v2.creatorProfiles';
...
const DEFAULT_LOCAL_CREATOR_PROFILE = {
  id: 'you',
  name: 'You',
...
function loadCreatorProfiles() {
  try {
    ...
    const normalized = parsed.map((profile) => normalizeCreatorProfile(profile)).filter(Boolean);
...
function saveCreatorProfiles(profiles) {
  const safe = Array.isArray(profiles)
    ? profiles.map((profile) => normalizeCreatorProfile(profile)).filter(Boolean)
    : [DEFAULT_CREATOR_PROFILE, DEFAULT_LOCAL_CREATOR_PROFILE];
...
function upsertCreatorProfile(profile) {
  ...
  const next = loadCreatorProfiles().filter((item) => item.id !== normalized.id);
...
function getCreatorProfileById(id, profiles = null) {
  if (!id) return null;
  const current = Array.isArray(profiles) ? profiles : loadCreatorProfiles();
...
function getTrendingScore({ likes = 0, uses = 0, createdAt = null, updatedAt = null } = {}) {
  const likeScore = Math.max(0, Number(likes) || 0) * 3;
  const useScore = Math.max(0, Number(uses) || 0) * 2;
  const refTime = updatedAt || createdAt || null;
  const recentBoost = refTime
    ? Math.max(0, 30 - Math.min(30, Math.floor((Date.now() - new Date(refTime).getTime()) / 86400000)))
    : 0;
  return likeScore + useScore + recentBoost;
}

function buildCreatorSharePayload({ framePresetId = '', packId = '', creatorId = '' } = {}) {
  return {
