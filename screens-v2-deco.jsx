...
function DecoV2({ T, go, mobile, variant, shots, selected, filter, layout, orientation,
  ...
  React.useEffect(() => {
    ...
    const draw = async () => {
      ...
      const presetCanvas = framePreset?.canvasSize || null;
      const baseW = presetCanvas?.width || template.canvasSize.width;
      const baseH = presetCanvas?.height || template.canvasSize.height;
...
function ResultV2({ T, go, mobile, variant, shots, selected, filter, layout, orientation, stickers, drawStrokes, logo, dateText, accent, frameColor, framePreset, activeSessionId, openDesigner }) {
    ...
    try {
      ...
      if (window.validateExportAssets) {
        ...
        if (!valResult.ok) {
          ...
          const hasBackgroundError = valResult.failures.some(f => f.type === 'background');
  ...
  const handleVideoDownload = async () => {
    ...
    const presetCanvas = framePreset?.canvasSize || null;
    const baseW = presetCanvas?.width || template?.canvasSize?.width || 720;
    const baseH = presetCanvas?.height || template?.canvasSize?.height || 960;
    const W = 720;
