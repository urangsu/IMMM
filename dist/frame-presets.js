...
var FRAME_PRESET_STORAGE_KEY = 'immm.v2.customFrames';
...
var FRAME_SYSTEM_LAYER_TYPES = ['background', 'photo-slots', 'watermark'];
...
var DEFAULT_CUSTOM_AUTHOR = {
  name: 'You',
  handle: '',
  url: ''
};
var DEFAULT_IMPORTED_AUTHOR = {
  name: 'Imported',
  handle: '',
  url: ''
};
...
function normalizeAuthor(author, fallback = DEFAULT_FRAME_AUTHOR) {
  var source = author && typeof author === 'object' ? author : null;
  return {
...
function normalizeLicense(license) {
  var value = String(license || DEFAULT_FRAME_LICENSE).toLowerCase();
...
function writeStringArrayStorage(key, values) {
  var safe = Array.isArray(values) ? values.map(item => String(item || '')).filter(Boolean) : [];
...
function normalizeFrameLayers(layers) {
  var defaults = buildDefaultFrameLayers();
  var source = Array.isArray(layers) && layers.length ? layers : defaults;
  var byType = new Map();
  source.forEach((layer, index) => {
    var normalized = createFrameLayer(layer, index);
  ...
  return defaults.map((fallbackLayer, index) => {
    var incoming = byType.get(fallbackLayer.type) || {};
    return createFrameLayer({
      ...fallbackLayer,
      ...incoming,
...
function getPhotoSlotsForLayout(layout) {
  var normalized = normalizePresetLayout(layout);
  var size = getCanvasSizeForLayout(normalized);
...
function clampRectToCanvas(rect, canvasSize, minWidth = 48, minHeight = 48) {
  var width = Number(canvasSize?.width) || 1;
  var height = Number(canvasSize?.height) || 1;
  var nextWidth = clampNumber(rect?.width ?? minWidth, minWidth, width);
  var nextHeight = clampNumber(rect?.height ?? minHeight, minHeight, height);
  var nextX = clampNumber(rect?.x ?? 0, 0, Math.max(0, width - nextWidth));
  var nextY = clampNumber(rect?.y ?? 0, 0, Math.max(0, height - nextHeight));
  return {
    x: nextX,
    y: nextY,
    width: nextWidth,
...
function createFrameDesignerDraft(basePreset = null) {
  var preset = normalizeFramePreset(basePreset || getBuiltinFramePresets()[0] || null) || normalizeFramePreset({
  ...
  var createdAt = preset.createdAt || new Date().toISOString();
...
function normalizeDesignerDraft(draft) {
  ...
  var layout = normalizePresetLayout(draft.layout || '1x4');
  var canvasSize = clonePlain(draft.canvasSize || getCanvasSizeForLayout(layout));
...
function validateDesignerDraft(draft) {
  var normalized = normalizeDesignerDraft(draft);
  if (!normalized) {
    return {
      ok: false,
      error: 'Designer draft is invalid'
    };
  }
  ...
  if (json.includes('dataUrl') || json.includes('data:image/')) {
    return {
      ok: false,
      error: 'Designer draft must not contain dataUrl'
    };
  }
  if (json.length > 1024 * 1024) {
    return {
      ok: false,
      error: 'Designer draft is too large'
    };
  }
  return {
    ok: true,
...
function draftToCustomFramePreset(draft) {
  var validation = validateDesignerDraft(draft);
  if (!validation.ok) return null;
  var normalized = validation.draft;
...
function duplicateFramePresetAsDraft(preset) {
  var normalized = normalizeFramePreset(preset);
  if (!normalized) return null;
  var draft = createFrameDesignerDraft(normalized);
  if (!draft) return null;
...
function makePreset({
  ...
  return {
    ...
    photoSlots: Array.isArray(photoSlots) && photoSlots.length ? photoSlots.map(slot => ({
      ...slot
    })) : getPhotoSlotsForLayout(resolvedLayout),
...
function buildBuiltinFramePresets() {
  var stripSize = getCanvasSizeForLayout('1x4');
  var gridSize = getCanvasSizeForLayout('2x2');
  var tripSize = getCanvasSizeForLayout('1x3');
  var polaroidSize = getCanvasSizeForLayout('1x1');
  return [makePreset({
    id: 'clean-white-4cut',
...
var BUILTIN_FRAME_PACKS = [makePack({
  id: 'basic-clean-pack',
...
function loadCustomFramePresets() {
  try {
    var raw = localStorage.getItem(FRAME_PRESET_STORAGE_KEY);
    if (!raw) return [];
    var parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.map(preset => normalizeFramePreset({
      ...preset,
      source: preset.source === 'imported' ? 'imported' : 'custom'
...
function saveCustomFramePresets(frames) {
  var safe = (Array.isArray(frames) ? frames : []).map(preset => {
...
function getImportedFramePackGroups(customFrames = []) {
  ...
  (Array.isArray(customFrames) ? customFrames : []).forEach(frame => {
    ...
    var normalized = normalizeFramePreset({
      ...frame,
      source: 'imported'
    });
    var packId = normalized.packId || `imported-${normalized.id}`;
...
function getFramePresetsByPack(packId, customFrames = []) {
  if (!packId) return [];
  var pack = getFramePackById(packId, customFrames);
  if (!pack) return [];
  var presetIds = Array.isArray(pack.presetIds) ? pack.presetIds : [];
  var all = listFramePresets(customFrames);
  return presetIds.map(presetId => all.find(preset => preset.id === presetId) || null).filter(Boolean);
}
function getUnlockedFramePackIds() {
  var defaults = BUILTIN_FRAME_PACKS.filter(pack => !pack.locked).map(pack => pack.id);
...
function isFramePackUnlocked(packId) {
  ...
  var pack = BUILTIN_FRAME_PACKS.find(item => item.id === packId);
  if (!pack) return true;
...
function unlockFramePackForDev(packId) {
  if (!packId) return [];
  var next = mergeUniqueStrings(getUnlockedFramePackIds(), [packId]);
...
function toggleFavoriteFramePresetId(presetId, current = null) {
  if (!presetId) return [];
  var existing = Array.isArray(current) ? current : loadFavoriteFramePresetIds();
  var next = existing.includes(presetId) ? existing.filter(id => id !== presetId) : [...existing, presetId];
  saveFavoriteFramePresetIds(next);
  return next;
}
function isFavoriteFramePresetId(presetId, favorites = null) {
  if (!presetId) return false;
  var current = Array.isArray(favorites) ? favorites : loadFavoriteFramePresetIds();
...
function validateFramePackJson(raw) {
  ...
  if (raw.length > 1024 * 1024) {
    return {
      ok: false,
      error: 'Frame pack JSON too large'
    };
  }
  var parsed;
  try {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return {
      ok: false,
      error: 'Frame pack must be an object'
    };
  }
  if (framePackHasDataUrl(parsed)) {
    return {
      ok: false,
      error: 'Frame pack JSON must not contain dataUrl'
    };
  }
  var pack = parsed.pack && typeof parsed.pack === 'object' ? parsed.pack : {};
  var presets = Array.isArray(parsed.presets) ? parsed.presets : [];
  if (!Array.isArray(presets) || presets.length === 0) {
    return {
      ok: false,
      error: 'Frame pack has no presets'
    };
  }
  var normalizedPack = makePack({
  ...
  if (framePackHasDataUrl(pack)) {
    return {
      ok: false,
      error: 'Frame pack metadata must not contain dataUrl'
    };
  }
  if (!normalizedPack.coverPresetId || !normalizedPack.presetIds.includes(normalizedPack.coverPresetId)) {
    return {
      ok: false,
      error: 'Frame pack cover must be one of the preset ids'
    };
  }
  return {
    ok: true,
...
function exportCustomFramePackJson(frames = [], options = {}) {
  ...
  var presets = (Array.isArray(frames) ? frames : []).filter(frame => frame && frame.source !== 'builtin' && !frame.deletedAt).map(frame => sanitizeCustomFramePreset({
    ...frame,
    source: frame.source === 'imported' ? 'imported' : 'custom'
  })).filter(Boolean);
  var pack = makePack({
...
function importFramePackJson(raw) {
  var validation = validateFramePackJson(raw);
  if (!validation.ok) {
    return {
      ok: false,
  ...
  var existingIds = new Set(listFramePresets().map(preset => preset.id));
  var remap = new Map();
  var nextPresets = presets.map((frame, index) => {
    var nextId = String(frame.id || '').trim() || `imported-${index}-${Date.now().toString(36)}`;
    var uniqueId = existingIds.has(nextId) || remap.has(nextId) ? `imported-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}` : nextId;
    remap.set(frame.id, uniqueId);
    existingIds.add(uniqueId);
    var sanitized = sanitizeImportedFramePreset({
      ...frame,
      id: uniqueId,
      source: 'imported'
    }, pack);
  ...
  if (nextPresets.length === 0) {
    return {
      ok: false,
      error: 'No valid presets to import'
    };
  }
  return {
    ok: true,
...
function sanitizeDrawStroke(stroke) {
  ...
  var points = Array.isArray(stroke.points) ? stroke.points.filter(pt => Array.isArray(pt) && pt.length >= 2).map(([x, y]) => [Number(x) || 0, Number(y) || 0]) : [];
...
function sanitizeCustomFramePreset(frame) {
  ...
  var decorations = Array.isArray(frame.decorations) ? frame.decorations.map(deco => makeDecoration(deco)).filter(Boolean) : [];
  var stickers = Array.isArray(frame.stickers) ? frame.stickers.map(sticker => sanitizeFrameSticker(sticker)).filter(Boolean) : [];
  var drawStrokes = Array.isArray(frame.drawStrokes) ? frame.drawStrokes.map(stroke => sanitizeDrawStroke(stroke)).filter(Boolean) : [];
  var background = frame.background ? clonePlain(frame.background) : createBackground('solid', frame.frameColor || '#FFFFFF');
  var preset = normalizeFramePreset({
...
function createCustomFramePresetFromAppState(input = {}) {
  var now = new Date().toISOString();
  var frame = sanitizeCustomFramePreset({
    id: input.id,
...
function listFramePresets(customFrames = []) {
  return [...getBuiltinFramePresets(), ...customFrames.map(preset => normalizeFramePreset({
    ...preset,
    source: preset.source === 'imported' ? 'imported' : 'custom'
  })).filter(preset => preset && !preset.deletedAt)];
}
...
function getFramePresetsByCategory(category, customFrames = []) {
  if (category === 'my-frames') {
    return (customFrames || []).map(preset => normalizeFramePreset({
      ...preset,
      source: preset.source === 'imported' ? 'imported' : 'custom'
    })).filter(preset => preset && !preset.deletedAt);
  }
...
function drawFrameDecoration(ctx, deco, width, height) {
  ...
  var x = deco.x * width;
  var y = deco.y * height;
  var w = deco.width * width;
  var h = deco.height * height;
  ctx.save();
  ...
  if (deco.type === 'text') {
    var fontSize = Math.max(10, Math.min(160, Math.min(w, h)));
...
function drawFramePresetBackground(ctx, preset, width, height) {
  ...
  if (background.type === 'solid') {
    ...
    var cfg = background.value || {};
    var gradient;
    if (cfg.type === 'radial') {
      gradient = ctx.createRadialGradient(width * 0.5, height * 0.44, 0, width * 0.5, height * 0.44, Math.max(width, height) * 0.65);
    } else {
      var angle = Number(cfg.angle || 0) * Math.PI / 180;
      var dx = Math.cos(angle) * width * 0.5;
      var dy = Math.sin(angle) * height * 0.5;
      gradient = ctx.createLinearGradient(width * 0.5 - dx, height * 0.5 - dy, width * 0.5 + dx, height * 0.5 + dy);
    }
    var stops = Array.isArray(cfg.stops) && cfg.stops.length ? cfg.stops : ['#FFFFFF', '#F6F6F6'];
    var steps = Math.max(1, stops.length - 1);
    stops.forEach((stop, idx) => gradient.addColorStop(steps === 0 ? 0 : idx / steps, stop));
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, width, height);
  } else if (background.type === 'pattern') {
    var _cfg = background.value || {};
    ctx.fillStyle = _cfg.color || '#FFFFFF';
    ctx.fillRect(0, 0, width, height);
    var pattern = _cfg.pattern || 'dots';
    if (pattern === 'dots') {
      ctx.fillStyle = _cfg.dotColor || 'rgba(0,0,0,0.04)';
      var step = Math.max(30, Math.min(width, height) * 0.06);
      ...
      var colors = _cfg.accentColors || ['#FF8F9B', '#FFD66B', '#9FD3FF'];
      for (var i = 0; i < 110; i++) {
        var _x = i * 97 % width;
        var _y = i * 53 % height;
        ctx.save();
      ...
      ctx.fillStyle = _cfg.bubbleColor || 'rgba(0,0,0,0.05)';
      for (var _i = 0; _i < 34; _i++) {
        var _x2 = _i * 83 % width;
        var _y2 = _i * 127 % height;
        var r = 10 + _i % 5 * 6;
        ctx.beginPath();
        ctx.arc(_x2, _y2, r, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (background.type === 'image') {
      var src = background.value;
      if (src) {
        var cachedImg = typeof window !== 'undefined' && window.__IMMM_BACKGROUND_IMAGE_CACHE__ && window.__IMMM_BACKGROUND_IMAGE_CACHE__.get(src);
        if (cachedImg) {
          ctx.save();
          var ar = cachedImg.width / cachedImg.height;
          var dar = width / height;
          var sx = 0,
            sy = 0,
            sw = cachedImg.width,
            sh = cachedImg.height;
...
function getFramePresetCategoriesWithCounts(customFrames = []) {
  return FRAME_PRESET_CATEGORIES.map(category => {
    if (category.id === 'my-frames') {
      return {
        ...category,
        count: (customFrames || []).filter(preset => !preset.deletedAt).length
      };
    }
    return {
      ...category,
...
var FRAME_PACKS = BUILTIN_FRAME_PACKS;
var IMMMFramePresets = {
