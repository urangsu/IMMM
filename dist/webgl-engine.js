...
var FILTER_PIPELINES = {
  ...
  porcelain: {
    pipeline: [{
      ...
      shader: 'split_tone',
      uniforms: {
        u_shadowColor: [0.96, 0.97, 1.02],
        u_highlightColor: [1.02, 1.01, 0.98],
        u_intensity: 0.38
      }
    }, {
      shader: 'color_adjust',
  ...
  smooth: {
    pipeline: [{
      ...
      shader: 'split_tone',
      uniforms: {
        u_shadowColor: [0.95, 0.96, 1.02],
        u_highlightColor: [1.03, 1.015, 0.99],
        u_intensity: 0.38
      }
    }, {
      shader: 'color_adjust',
  ...
  blush: {
    pipeline: [{
      ...
      shader: 'blush',
      uniforms: {
        u_blushStrength: 0.65,
        u_blushColor: [1.0, 0.50, 0.55]
      }
    }]
  }
};
...
class FilterEngine {
  ...
  _init() {
    ...
    this._maskTex = twgl.createTexture(gl, {
      width: 1,
      height: 1,
  ...
  _pass(shaderName, inputTex, outputFbo, uniforms) {
    ...
    if (outputFbo) gl.viewport(0, 0, outputFbo.width, outputFbo.height);else gl.viewport(0, 0, this.canvas.width, this.canvas.height);
  ...
  _onRestored() {
    this._init();
    if (this._getSource) this.startLoop(this._getSource, this._getParams, this._getSize, null);
  }
}
