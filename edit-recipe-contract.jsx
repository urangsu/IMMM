...
(function() {
  const EDIT_CONTRACT_VERSION = '1.0.0';
  ...
  const BLUR_TYPES = Object.freeze([
    'background',
    'face-safe',
    'full-frame'
  ]);
  ...
  const createBlurRecipe = (params) => {
    ...
    const finalBlurType = BLUR_TYPES.includes(blurType) ? blurType : 'background';
    const finalStrength = Math.min(1.0, Math.max(0, strength));
    ...
    return Object.freeze({
      ...
      radius: finalRadius,
      metadata: isPlainObject(metadata) ? Object.freeze({ ...metadata }) : Object.freeze({})
    });
  };
   ...
   */
  const createFilterRecipe = (params) => {
    const {
      filterId = 'original',
      intensity = 1.0,
      metadata = {}
    ...
    const finalIntensity = Math.min(1.0, Math.max(0, intensity));
   ...
   */
  const createCompositeEditRecipe = (params) => {
    const {
      edits = [],
      metadata = {}
    } = params || {};

    const finalEdits = Array.isArray(edits) ? edits : [];
   ...
   */
  const validateEditRecipe = (recipe) => {
    const errors = [];
   ...
   */
  const runEditRecipeContractSelfTest = () => {
    const errors = [];

    // Test 1: Blur recipe creation
    const blurRecipe = createBlurRecipe({
      blurType: 'background',
      strength: 0.7
    });
    ...
    const blurValidation = validateEditRecipe(blurRecipe);
    ...
    const invalidBlurRecipe = createBlurRecipe({
    ...
    const filterRecipe = createFilterRecipe({
      filterId: 'porcelain',
      intensity: 0.8
    });
    const filterValidation = validateEditRecipe(filterRecipe);
    if (!filterValidation.ok) {
      errors.push('Test 4 FAIL: Valid filter recipe failed validation');
    }

    // Test 5: Crop recipe
    const cropRecipe = createCropRecipe({
      x: 10,
      y: 20,
      width: 300,
      height: 400
    });
    const cropValidation = validateEditRecipe(cropRecipe);
    if (!cropValidation.ok) {
      errors.push('Test 5 FAIL: Valid crop recipe failed validation');
    }

    // Test 6: Composite recipe
    const compositeRecipe = createCompositeEditRecipe({
    ...
    const strongBlur = createBlurRecipe({ strength: 5.0 });
