...
- [ ] **PWA 설치 자산 추가**: `manifest.webmanifest`는 있으나 `icon.svg`, `sw.js`가 없어 404가 난다. 설치 가능 앱으로 보이려면 아이콘과 최소 서비스워커가 필요하다.
- [ ] **모바일 WebGL 정책 확정**: 현재 모바일 WebGL은 검정 화면 방지를 위해 비활성화했다. 얼굴 추적 필터를 모바일에서도 살릴지, 안정성을 우선해 Canvas2D 기반으로 갈지 결정해야 한다.
- [ ] **필터 UX 재설계**: 현재 필터는 안정성을 위해 축소했다. 실제 카메라 앱/SNOW 기준으로 `피부보정`, `홍조`, `프리쿠라`, `필름`처럼 목적 중심 4~6개로 재구성하고 샘플 이미지를 재검토한다.
- [ ] **순수 캔버스 Export 파이프라인**: `html2canvas` 의존도를 낮추고 촬영 컷, 프레임, 스티커를 하나의 고해상도 캔버스에 직접 그려 저장 (보이는 화면과 저장본의 100% 일치 및 품질 보장).
